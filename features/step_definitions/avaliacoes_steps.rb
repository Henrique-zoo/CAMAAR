...
# == Descrição
# Método auxiliar para os testes que cria a associação entre um usuário e uma turma.
# Se o tipo de participação for discente, cria também o PerfilDiscente associado com uma matrícula gerada aleatoriamente.
#
# == Argumentos
# * +usuario+: Instância do modelo Usuario.
# * +turma+: Instância do modelo Turma.
# * +tipo+: Symbol representando o tipo de participação (ex: +:discente+).
#
# == Retorno
# * Retorna o objeto +ParticipacaoTurma+ criado ou encontrado no banco.
#
# == Efeitos Colaterais
# * *Banco de Dados (Escrita)*: Cria registros na tabela +ParticipacaoTurma+ e, condicionalmente, na tabela +PerfilDiscente+.
...
# == Descrição
# Step definition que prepara o banco com um usuário de teste e seu respectivo perfil discente.
...
# == Descrição
# Step definition que simula a autenticação do usuário através de mock do RSpec no ApplicationController.
...
# == Descrição
# Step definition que cria a hierarquia completa de Departamento, Materia e Turma, e matricula o usuário nela.
...
# == Descrição
# Step definition que cria um formulário pendente vinculado a uma turma e gera a avaliação para o participante.
